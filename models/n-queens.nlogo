...
globals [
  ...
  Solutions     ; will store query solutions.
  eof-sol       ; boolean to knoe if we reach the last solution
  ]

; Procedure to create a new board of size N
...
to create-board [N]
  ...
  ask patches with [(pxcor + pycor) mod 2 = 0] [
    set pcolor white]
  ; Create Queens as turtles
  create-turtles board-size [
...
; Prolog Initialization Procedure
...
; Procedure to get the next solution from the query

to next
  ; If we have reached the last recovered solution from the query, we stop
  if eof-sol [stop]
  ; We check if there is one more solution to be recovered
  ifelse netprologo:next-stored-solution Solutions
  [
...
Content of nqueens.pl file (Prolog source code):
