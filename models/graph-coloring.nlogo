; Don't forget to load the NetProLogo extension
; (see the extension doc for the meaning of instructions)
...
to initialize
  ...
  build-graph
  ; Third, we build the facts on Prolog
  load-facts
  ; Finally, we launch the findall query to obtain the solutions
  if not netprologo:run-query "findall(N, vertex(N), LN),findall(C, color(C), LC),vertexColoring(LN,LC,[],R)"
    [user-message "Not enough colors to find a solution." stop]
end

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; Procedure to build the Knowledge Base in Prolog
...
; Creating the Colors KB
to load-color-facts
  ;; Clean Colors Knowledge Base by using retractall
...
to load-vertex-facts
  ...
  [
    ...
    if not netprologo:run-query (netprologo:build-prolog-call "assert(vertex(?1))" who)
    [show "Error loading vertex facts."]
  ]
end

; Creating the Arcs KB
to load-arc-facts
  ...
  [
    ...
    if not netprologo:run-query (netprologo:build-prolog-call "assert(arc(?1,?2))" [who] of end1 [who] of end2)
    [show "Error loading arc facts."]
  ]
end
...
to next-coloring
  ...
  [user-message "No more solutions." stop]
  [
    ;; If any, get variable value for current solution.
...
; Procedure to create the graph in NetLogo
to build-graph
  ; Create nodes
  set-default-shape turtles "circle"
  create-turtles numVertex
  [
    set color blue
    set size 3
  ...
  ask turtles
  [
    create-links-with n-of (ceiling random-float (arcProbability * numVertex / 100)) other turtles
  ]
